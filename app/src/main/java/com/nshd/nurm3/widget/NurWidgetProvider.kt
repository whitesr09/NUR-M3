...
open class NurWidgetProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == AppWidgetManager.ACTION_APPWIDGET_UPDATE || intent.action == AppWidgetManager.ACTION_APPWIDGET_OPTIONS_CHANGED) refresh(context)
    }
    companion object {
        const val EXTRA_ROUTE = "com.nshd.nurm3.ROUTE"
        private val providers = listOf(NurWidgetProvider::class.java, NurPrayerWidgetProvider::class.java, NurTaskWidgetProvider::class.java, NurQuickWidgetProvider::class.java)
        fun refresh(context: Context) {
            val app = context.applicationContext
            CoroutineScope(SupervisorJob() + Dispatchers.IO).launch { runCatching { refreshNow(app) } }
        }
        ...
        private fun shortcut(context: Context, widgetId: Int, action: Int, route: String): PendingIntent {
            ...
            return PendingIntent.getActivity(context, widgetId * 10 + action, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        }
        private fun updateWidget(context: Context, manager: AppWidgetManager, id: Int, type: String, entries: List<Entry>, completions: List<Completion>, today: LocalDate, prefs: NurPreferences) {
            ...
            val summary = if (type == "daily" || type == "quick") DailyProgress.lightSummary(entries, completions, today) else DailyProgress.summary(selected, completions, today)
            val title = when (type) { "prayer" -> "Prayer"; "task" -> "Amanah"; "quick" -> "Quick actions"; else -> "نُور" }
            ...
            val views = RemoteViews(context.packageName, R.layout.nur_daily_widget).apply {
                ...
                setTextViewText(R.id.widget_date, today.format(DateTimeFormatter.ofPattern("d MMM")))
                setTextViewText(R.id.widget_subtitle, if (prefs.widgetsEnabled) subtitle else "Widgets are disabled in NUR settings")
                setProgressBar(R.id.widget_progress, 100, (summary.fraction * 100).toInt().coerceIn(0, 100), false)
                ...
                setOnClickPendingIntent(R.id.widget_action_one, shortcut(context, id, 1, if (type == "prayer") "journey" else "amanah?create=true"))
...
class NurWidgetRefreshReceiver : android.content.BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) { NurWidgetProvider.refresh(context) }
}
