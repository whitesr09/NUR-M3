...
class NurRefreshController(private val window: Window, context: Context) : DefaultLifecycleObserver {
    ...
    private fun apply() {
        ...
        if (display == null) {
            _status.value = "Display information unavailable"
            return
        }
        val choice = RefreshRatePolicy.choose(display.supportedModes, display.mode)
        if (choice == null) {
            restore()
            _status.value = "No compatible display mode reported"
            return
        }

        val current = window.attributes
        if (
            current.preferredDisplayModeId != choice.modeId ||
            current.preferredRefreshRate != choice.refreshRate
        ) {
            window.attributes = current.apply {
                preferredDisplayModeId = choice.modeId
                preferredRefreshRate = choice.refreshRate
            }
        }
        updateStatus()
    }
    ...
    private fun restore() {
        val current = window.attributes
        if (
            current.preferredDisplayModeId != originalMode ||
            current.preferredRefreshRate != originalRate
        ) {
            window.attributes = current.apply {
    ...
    private fun updateStatus() {
        ...
        _status.value = if (requested && resumed) {
            ...
            "System controlled · Display reports ${format(current)} Hz"
        }
    }
